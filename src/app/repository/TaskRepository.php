...
class TaskRepository {
    ...
    public function findOneById($connection, $id) {
        $stmt = mysqli_prepare($connection, "SELECT `name`,`description` FROM `tm`.`tm_task` WHERE id = ?; ");
    ...
    public function createOneByDefault($connection) {
        $name = 'New Task';
        $description = '';
        $stmt = mysqli_prepare($connection, "INSERT INTO `tm`.`tm_task` (`name`,`description`) VALUES (?,?); ");
    ...
    public function updateOneById($connection, $id, $name, $description) {
        $stmt = mysqli_prepare($connection, "UPDATE `tm`.`tm_task` SET `name` = ?, `description` = ? WHERE `id` = ? ");
